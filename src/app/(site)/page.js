...
export default async function HomePage() {
  ...
  const grid = (
    featured.length > 1 ? featured.slice(1, 7) : products.slice(1, 7)
  ).filter(Boolean);
      ...
      {spotlight && (
        <section className="relative mx-auto max-w-7xl px-5 py-20 sm:px-8">
          <div
            className="pointer-events-none absolute left-1/2 top-1/3 h-96 w-96 -translate-x-1/2 rounded-full opacity-25 animate-blob"
            style={{
              background: "radial-gradient(circle,#ffb15c,transparent 65%)",
            }}
          />
      ...
      <section className="mx-auto max-w-7xl px-5 py-16 sm:px-8">
        <div className="grid gap-6 sm:grid-cols-3">
          {[
            {
              icon: "🚚",
              title: "Fast marketplace delivery",
              copy: "Prime, Flipkart Plus and Meesho Next Day — you pick the speed.",
            },
            {
              icon: "🔁",
              title: "7-day easy returns",
              copy: "Every listing follows the marketplace return policy. No arguments.",
            },
            {
              icon: "🇮🇳",
              title: "Made in India",
              copy: "Cut, stitched and finished in Surat by a team we know by name.",
            },
          ].map((item, index) => (
          ...
          <div className="flex flex-col items-start justify-between gap-6 rounded-[32px] border border-white/10 bg-ink-2/60 p-9 sm:flex-row sm:items-center">
            <div>
              <p className="text-[0.6rem] uppercase tracking-[0.35em] text-gold">
                For the VIMUHET team
              </p>

              <h3 className="mt-3 font-display text-3xl">
                Upload products, track every click
              </h3>

              <p className="mt-2 max-w-xl text-sm text-cream/50">
                The admin panel adds styles, attaches Amazon / Flipkart / Meesho
                links and shows live visitor and marketplace-click analytics.
              </p>
            </div>
