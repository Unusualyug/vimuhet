    ...
    <div className="min-h-screen bg-ink text-cream">
      <div className="pointer-events-none fixed inset-0 -z-10">
        ...
        <div
          ...
          style={{ background: "radial-gradient(circle,#ff5d8f,transparent 65%)", animationDelay: "-8s" }}
        />
      </div>
        ...
        <aside className="lg:sticky lg:top-0 lg:h-screen lg:w-72 lg:shrink-0 lg:border-r lg:border-white/8">
          <div className="flex items-center justify-between gap-4 p-6">
            <Link href="/" className="flex items-center gap-3">
              <span
                className="flex h-9 w-9 items-center justify-center rounded-xl font-display text-lg font-bold text-ink animate-grad"
                style={{ background: "var(--grad)" }}
              >
                V
              </span>
              <span>
                <span className="block font-display text-lg tracking-[0.18em]">VIMUHET</span>
                <span className="block text-[0.55rem] uppercase tracking-[0.3em] text-cream/35">Admin console</span>
              </span>
            </Link>
          </div>
              ...
              return (
                <Link
                  ...
                  className={`relative flex shrink-0 items-center gap-3 rounded-xl px-4 py-3 text-sm transition-colors ${
                    active ? "text-ink" : "text-cream/60 hover:bg-white/5 hover:text-cream"
                  }`}
                >
                  {active && (
                    <motion.span
          ...
          <div className="hidden px-6 lg:block">
            <div className="rounded-2xl border border-white/8 bg-ink-2/60 p-4">
              <p className="text-[0.55rem] uppercase tracking-[0.28em] text-gold">Image storage</p>
              <p className="mt-2 text-xs leading-relaxed text-cream/50">{storageNote}</p>
            </div>
            <button
              type="button"
              onClick={logout}
        ...
        <main className="min-w-0 flex-1 px-5 pb-20 pt-6 sm:px-8">
          <header className="mb-8 flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
            <div>
              ...
              {subtitle && <p className="mt-3 max-w-2xl text-sm text-cream/45">{subtitle}</p>}
            </div>
            <button
              type="button"
              onClick={logout}
