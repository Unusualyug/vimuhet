...
export async function ensureSeed() {
  ...
  // Prevent multiple requests from trying to seed at the same time.
  if (globalForStore.__vimuhetSeedPromise) {
    return globalForStore.__vimuhetSeedPromise;
  }
  ...
  globalForStore.__vimuhetSeedPromise = (async () => {
    ...
    // IMPORTANT:
    // Drizzle does not allow .values([]).
    // If seedProducts is empty, simply skip the insert.
    if (!Array.isArray(seedProducts) || seedProducts.length === 0) {
      console.warn(
        "Warning: seedProducts is empty. No products were inserted.",
      );
      return;
    }
  ...
  try {
    return await globalForStore.__vimuhetSeedPromise;
  } finally {
    globalForStore.__vimuhetSeedPromise = null;
  }
}
...
export async function listProducts(options = {}) {
  ...
  if (!includeInactive) {
    filters.push(eq(products.active, true));
  }
...
export async function uniqueSlug(name, currentId) {
  ...
  for (;;) {
    ...
    if (
      !rows.length ||
      (currentId && Number(rows[0].id) === Number(currentId))
    ) {
      return candidate;
    }
...
export async function getStats() {
  ...
  const seriesRes = await db.execute(sql`
    select
      ...
      (
        select count(*)
        from site_visits v
        where date_trunc('day', v.created_at) = d.day
      ) as visits,

      (
        select count(*)
        from product_clicks c
        where date_trunc('day', c.created_at) = d.day
      ) as clicks
  ...
  const hourlyRes = await db.execute(sql`
    select
      to_char(d.hour, 'HH24') as label,

      (
        select count(*)
        from site_visits v
        where date_trunc('hour', v.created_at) = d.hour
      ) as visits
